...
例如我们在组件A中，使用一个字符串类型的 key，作为变量 a 的 ID，然后定义变量 a，同时得到变量 a 的引用。
...
第3步：在组件B中，使用 Recoil 提供的不同的钩子函数(hooks)，将变量 a 的引用对象作为参数 ，获取变量 a 的不同权限。
...
> 我认为一定要有一套变量名命名规范，确保通过 key 的字面意思就能知道该变量的作用和意义。
...
**Recoil还可以通过派生/衍生，将变量进行更加灵活的拆分或组合。**

**什么是派生？什么是衍生？**

> ```
> 首先声明，以下关于 派生和衍生 的解释，都是我自己胡说八道的，你听听就好。
> 
> 在编程思想里：
> 派生：由某一根本事物的发展过程中分化出来的东西。
> 衍生：通过演变而产生的东西，实际上或理论上从母体物质得到的物质。
> 
> 我用 JS 举一个简单示例，你感受一下 派生 和 衍生。
> 
> //派生
> const obj = { a:'aa', b:'bb'}
> const { a } = obj // a 是由 obj 的一个属性 拆分 而来的
> 
> //衍生
> const a = { a:'aa' }
> const b = { b:'bb' }
> const obj = { a: a.a, b:b.b } // obj 是由 a 和 b 共同组合而来的
> 
> 我们在实际的状态管理中，就会遇到不断的  拆分 或 组合，得到我们需要的数据状态。
> ```



> 在 Recoil 官网中，使用的英文单词为：derived data
>
> 你可以忽略 “衍生”，直接认为 无论拆分还是组合，都是派生。



所谓变量的 派生/衍生，无非 2 个方向：
...
通过变量的派生/衍生，也就是将变量进行 拆分和组合，可以解放手脚和思想，让 React 项目各个模块(组件)之间的变量使用更加灵活。
...
Recoil 让数据状态既可以局部拆分、组合，又保持整体统一连贯，那么此时我们再看看 Recoil 的标志，就能够明白其中的设计含义了。

<br>

![](https://puxiao.com/temp/recoil-tutorial/imgs/p0.jpg)
